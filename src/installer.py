...
class Installer(Archiso):
    ...
    def chmod_server_run_sh(self):
        loguru.logger.info("Chmod server run.sh...")
        return os.system(f'chmod 7777 {self.installer_dir_location}/server/run.sh') == 0
    
    def chmod_gui_run_sh(self):
        loguru.logger.info("Chmod gui main.py...")
        return os.system(f'chmod 7777 {self.installer_dir_location}/main.py') == 0
